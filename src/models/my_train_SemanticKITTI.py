...
from network.my_BEV_Unet import BEV_Unet
from network.my_ptBEV import ptBEVnet
...
def main(args):
    ...
    #prepare model
    my_BEV_model=BEV_Unet(n_class=len(unique_label), n_height = compression_model, dropout = 0.5, circular_padding = circular_padding)
    my_model = ptBEVnet(my_BEV_model, pytorch_device, grid_size, fea_dim = 7, out_pt_fea_dim = 512, max_pt_per_encode = 256, n_height = 32)
    ...
    while True:
        ...
        for i_iter,(_,train_vox_label,train_grid,_,train_pt_fea) in enumerate(train_dataset_loader):
            ...
            if global_iter % check_iter == 0:
                ...
                with torch.no_grad():
                    for i_iter_val,(_,val_vox_label,val_grid,val_pt_labs,val_pt_fea) in enumerate(val_dataset_loader):
                        ...
                        predict_labels = my_model(val_pt_fea_ten, val_grid_ten, circular_padding=circular_padding)
            ...
            try:
                ...
                outputs = my_model(train_pt_fea_ten,train_grid_ten, circular_padding=circular_padding)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--model_save_path', default='./my_SemKITTI_traditionalSeg.pt')
    parser.add_argument('-m', '--model', choices=['polar','traditional'], default='traditional', help='training model: polar or traditional')
