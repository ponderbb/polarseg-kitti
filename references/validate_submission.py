        ...
        with zipfile.ZipFile(FLAGS.zipfile) as zipfile:
            if FLAGS.task == "segmentation" or FLAGS.task == "panoptic":
                ...
                for sequence in range(11, 22):
                    ...
                    label_files = sorted(
                        [
                            os.path.join(sequence_directory, "predictions", os.path.splitext(filename)[0] + ".label")
                ...
                prediction_files = {
                    str(info.filename): info for info in zipfile.infolist() if not info.filename.endswith("/")
                }
